...
def scrape_pakwheels():
    ...
    for i in range(1, 101):  # Modify range as needed
...
def insert_into_database(cars_df):
    ...
    connection = pyodbc.connect(
        ...
        'SERVER=DESKTOP-J2KJLA0\\SQLEXPRESS;'  # Replace with your SQL Server name
        'DATABASE=FinalProject1;'  # Replace with your SQL database name
    ...
    cursor.execute(''' 
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='PakWheels_Cars_ISB' AND xtype='U')
    BEGIN
        CREATE TABLE PakWheels_Cars_ISB (
    ...
    for index, row in cars_df.iterrows():
        cursor.execute('''
            INSERT INTO PakWheels_Cars_ISB (Title, Price, Year, Mileage, Fuel_Type, Horsepower, Transmission, Page, Link)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', 
        row['TITLE'], 
        row['PRICE'], 
